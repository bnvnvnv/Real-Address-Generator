...
async function handleRequest(request) {
  ...
  if (requestedCountry && !COUNTRY_CODES.has(requestedCountry)) {
    return json({ error: 'invalid_country' }, { status: 400 })
  }

  const country = requestedCountry || getRandomCountry()
  const payload = await generateAddressPayload(country)

  if (!payload) {
    return json({ error: 'address_generation_failed' }, { status: 500 })
  }
